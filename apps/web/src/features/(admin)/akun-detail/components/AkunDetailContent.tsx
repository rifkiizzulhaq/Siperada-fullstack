...
import { useDeleteAkunDetail, useSearchAkunDetail } from "../hooks/useAkunDetail";
import { useAkunDetailStore } from "../store/akun-detail.store";
import AkunDetailForm from "./dialog/AkunDetailForm";
...
export default function AkunDetailContent() {
  const { openDialog } = useAkunDetailStore();
  ...
  const { data: res, isLoading, error } = useSearchAkunDetail({
  ...
  const deleteMutation = useDeleteAkunDetail(() => {
    queryClient.invalidateQueries({ queryKey: ["akun-detail-search"] });
    toast.success("Akun detail berhasil dihapus");
    setDeleteId(null);
  });
    ...
    <div className="w-full space-y-4">
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
        ...
        <div className="flex items-center gap-2 order-first sm:order-none sm:ml-auto">
          <Button size="sm" className="w-full sm:w-auto" onClick={() => openDialog()}>
            <Plus className="mr-2 h-4 w-4" /> Tambah Akun Detail
          </Button>
        </div>
      </div>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="cursor-pointer" onClick={() => handleSort("name")}>
                <div className="flex items-center">
                  Nama Akun Detail <SortIcon column="name" />
                </div>
              </TableHead>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow>
                <TableCell colSpan={4} className="h-24 text-center text-red-500 dark:text-white">
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              Apakah kamu yakin ingin menghapus akun detail ini?
            </AlertDialogTitle>
            <AlertDialogDescription>
